...
export default function PollNotificationListener() {
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (data.event === "started") {
          ...
          api.info({
            ...
            description:
              data.message ?? "A new poll has started for your group.",
            duration: 0,
            btn: url ? (
              <Button
                type="primary"
          ...
          if (
            typeof data.pollResultsUrl === "string" &&
            data.pollResultsUrl.trim()
          ) {
    ...
    // Re-run on pathname changes to connect after login
